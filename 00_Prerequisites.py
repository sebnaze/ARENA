...
# %%
eta = torch.Tensor([0.2, 0.5, 0.3])
T = torch.Tensor([[0, 1, 0], [0, 0, 1], [0.5, 0.5, 0]])
...
# %% 
# ----------------------------------------------#
# Exercices - einops oprations and broadcasting #
# ----------------------------------------------#
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    return temps - einops.repeat(temperatures_average(temps), 'w -> (w d)', d=7)
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    centered_temps = temperatures_differences(temps)
    std_temps = einops.reduce(temps, '(w d) -> w', t.std, d=7)
    std_temps = einops.repeat(std_temps, 'w -> (w d)', d=7)
    return centered_temps/std_temps
...
def normalize_rows(matrix: Tensor) -> Tensor:
    ...
    """
    l2 = matrix.norm(dim=1, keepdim=True)
    return matrix / l2
...
def sample_distribution(probs: Tensor, n: int) -> Tensor:
    ...
    """
    cprobs = probs.cumsum(dim=0)
    randoms = t.rand(n, 1)
    print((randoms>cprobs).shape)
    return (randoms>cprobs).sum(dim=-1)
...
def classifier_accuracy(scores: Tensor, true_classes: Tensor) -> Tensor:
    ...
    Use t.argmax.
    """
    return (scores.argmax(dim=-1) == true_classes).mean()
...
def total_price_gather(prices: Tensor, items: Tensor) -> float:
    ...
    gathered_prices = prices.gather(0, items)
    return gathered_prices.sum().item()
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    print(f"c = {c}")
    lse = t.log(t.exp(matrix - c).sum(dim=1, keepdim=True))
    print(f"lse = {lse}")
    res = (c+lse).squeeze()
    print(f"res = {res}")
    return res
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    true_log_probs = log_probs.gather(1, true_labels.unsqueeze(1)).squeeze()
