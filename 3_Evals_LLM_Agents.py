...
EVAL_MODEL = "openai/gpt-5.4-nano"
...
class ArithmeticTask:
    ...
    def _generate_answers(self) -> list[str]:
        ...
        for operation in self.operations:
            ...
            try:
                ...
                print(f"Error occurred while evaluating expression {expression}: {e}")
        return answers

    @property
    def get_current_task(self) -> str:
        """
        Returns the current task as a string in the format "num1 operation num2"

        Returns:
            str: The current task as a string
        """
        operation = self.operations[self.current_task_number]
        return f"{self.num1} {operation} {self.num2}"
    ...
    def get_current_instruction(self) -> ChatMessageUser:
        """
        Returns the current instruction as a ChatMessageUser object

        Returns:
            ChatMessageUser: The current instruction as a ChatMessageUser object
        """
        task_str = self.get_current_task
        return ChatMessageUser(content=f"Please compute the result of the following arithmetic operation: {task_str}. Give your answer in the format <ANSWER>NUMBER</ANSWER> where NUMBER is a numerical value formatted as a float.")
...
def calculate():
    async def execute(expression: str) -> str:
        ...
        Returns:
            str: The result of the evaluated expression, or an error message if the evaluation fails.
        """
        try:
...
def WikiAgent(tools: list[Tool], game: WikiGame):
    system_instruction = ChatMessageSystem(content=f"""You are a Wikipedia game agent.""")
    on_page_instruction = ChatMessageSystem(content=f"""You are currently on the page "{game.current_page.title}" and your goal is to reach the page "{game.goal_page.title}". You can use the following tools to help you navigate:
        - GetContentTool: Use this tool to retrieve the content of the current page. The content will have any accessible links wrapped in <link></link> tags.""")
    next_step_instruction = ChatMessageUser(content=f"""You need to navigate to the goal page. You can use the following tools to navigate to new pages:
        - MovePageTool: Use this tool to move to a new page by clicking on a link in the current page content. You can only move to pages which are wrapped in <link></link> tags in the content you retrieved using the GetContentTool. If you try to move to a page that is not permitted, the move will fail and you will remain on the current page.""")
    ...
    async def _handle_tool_calls(state: AgentState) -> AgentState:
        ...
        state.messages.extend(messages)
        if state.output.message.tool_calls[0].tool_name == "MovePageTool" and "success" in messages[-1].content.lower():
